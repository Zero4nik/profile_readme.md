...
**Тимур**, 8 месяцев активной разработки. Самоучка, первый курс ПГТК.
...
**Принцип:** изучаю новое без боязни забыть старое. Каждый проект — шаг вперёд, а не повтор пройденного. Докапываюсь до причины бага, а не засыпаю старших вопросами. Доделываю, даже если для этого приходится откатить изменения и написать всё заново.
...
|:---|:---|:---|
| **Deeplink** | Next.js 16, Express, Prisma, PostgreSQL, Redis, Socket.IO, JWT, Docker, GitHub Actions | Социальная сеть для разработчиков. Полноценный бэкенд с многослойной архитектурой, кэшированием, real-time уведомлениями, тестами и CI/CD |
| **vibeFM (бета)** | Next.js, TypeScript, PostgreSQL, Socket.IO, NextAuth.js | Синхронное прослушивание музыки с real-time чатом. OAuth через Google, сложная SQL-схема |
| **Social Network** | Node.js, Express, Socket.io, Redis, JWT, bcrypt | Lite-версия Discord: комнаты, real-time чат, регистрация с валидацией |
| **Faceless TodoList** | Next.js, TypeScript, Supabase, Framer Motion | Todo-приложение с CRUD, real-time синхронизацией через БД, фильтрацией и космическим UI |
| **Stripe Checkout** | Next.js 15, TypeScript, Stripe API, Vitest | Готовый платёжный модуль с тестами и деплоем на Vercel |
| **Whisper Box** | Next.js, TypeScript, Supabase | Приложение с сервисным уровнем, RLS и обновлениями в реальном времени |
| **Диванный Аналитик** | TypeScript, React | Аналитическая панель с фильтрацией, визуализацией данных и сложной клиентской логикой |
| **Личный сайт-портфолио** | React, Node.js, Telegram API | Визитка с анимациями Framer Motion и обратной связью через Telegram-бота |

---

## 🏆 Ключевые компетенции

**Архитектура:** проектирую многослойную структуру (Routes → Controllers → Services → Repositories), разделяю ответственность, пишу чистый код без `as any`.

**Backend:** JWT-аутентификация, CRUD с Prisma ORM, cursor-based пагинация, кэширование Redis с TTL и инвалидацией, WebSocket через Socket.IO, CORS, Helmet, Rate-limit.

**Frontend:** Next.js App Router, React 19, защищённые роуты, кастомные хуки, оптимистичное обновление UI с откатом при ошибках, Axios с интерсепторами, адаптивная вёрстка.

**DevOps:** Docker Compose (PostgreSQL + Redis), GitHub Actions (автопрогон тестов, проверка типов, миграции), деплой на Vercel (фронт) и Render (бэк).

**Тестирование:** Jest, Supertest, Vitest, Testing Library, Postman-коллекции с переменными окружения.

---

## 📊 Как я рос (хронология)

> **Декабрь 2025** — JavaScript, вёрстка, первые лендинги, LocalStorage, DOM-методы.
>
> **Январь 2026** — React, первый SPA (интернет-магазин), GitHub, Kwork, начало фриланса.
>
> **Февраль 2026** — портфолио на React + Node.js + Telegram API, проект WorkinLog, backend с Telegram API.
>
> **Март 2026** — Social Network (Node.js, Socket.io, Redis), Faceless TodoList (Next.js + Supabase), vibeFM (NextAuth.js, PostgreSQL).
>
> **Апрель 2026** — Stripe Checkout (тесты, CI/CD), **Deeplink** — флагманский проект с полным циклом разработки.
...
**Ищу работу:** Junior+ Frontend / Fullstack Developer  
**Формат:** удалённо  
**Зарплата:** от 95 000 ₽ на руки  
